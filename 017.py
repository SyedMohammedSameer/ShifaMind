#!/usr/bin/env python3
"""
ShifaMind Phase 4 vs Baseline Comparison
Comparing Phase 4 Revised against simple Bio_ClinicalBERT baseline

BASELINE MODEL:
- Bio_ClinicalBERT + Linear classifier
- No concepts, no fusion, no RAG
- Simple fine-tuning

PHASE 4 REVISED MODEL:
- Diagnosis-conditional concept labeling (PMI-based)
- Multi-layer cross-attention fusion
- Diagnosis-aware RAG
- Staged training (diagnosis → concepts → joint)

This comparison shows the value of the Phase 4 innovations.
"""
...
# ============================================================================
# BASELINE MODEL (Simple Bio_ClinicalBERT for comparison)
# ============================================================================
class BaselineModel(nn.Module):
    """
    Simple baseline: Bio_ClinicalBERT + linear classifier
    No concepts, no fusion, no RAG - just basic fine-tuning
    """
...
if __name__ == "__main__":
    ...
    # ==================================================================
    # TRAIN BASELINE MODEL (Simple Bio_ClinicalBERT)
    # ==================================================================
    print("\n" + "="*70)
    print("TRAINING BASELINE MODEL (Bio_ClinicalBERT)")
    print("="*70)

    # Create a fresh base model for baseline (not sharing with Phase 4)
    baseline_base_model = AutoModel.from_pretrained(model_name).to(device)
    baseline_model = BaselineModel(baseline_base_model, len(target_codes)).to(device)
    ...
    baseline_optimizer = torch.optim.AdamW(baseline_model.parameters(), lr=2e-5)
    ...
    test_preds = np.vstack(test_preds_all)
    test_labels = np.vstack(test_labels_all)
    ...
    print(f"  Train time:  {baseline_train_time:.1f}s ({baseline_train_time/60:.1f} min)")

    print(f"\n📊 Baseline Per-Class F1:")
    for i, code in enumerate(target_codes):
        print(f"  {code}: {baseline_results['per_class_f1'][i]:.4f}")

    # ==================================================================
    # TRAIN PHASE 4 REVISED MODEL
    # ==================================================================
    print("\n" + "="*70)
    print("TRAINING PHASE 4 REVISED MODEL")
    print("="*70)
    ...
    # ==================================================================
    # FINAL COMPARISON: BASELINE vs PHASE 4 REVISED
    # ==================================================================
    print("\n" + "="*70)
    print("FINAL COMPARISON: BASELINE vs PHASE 4 REVISED")
    print("="*70)
    ...
    print(f"\n📊 Per-Class F1 Scores:")
    print(f"  {'Code':<10} {'Baseline':>10} {'Phase 4':>10} {'Delta':>10}")
    print(f"  {'-'*10} {'-'*10} {'-'*10} {'-'*10}")
    for i, code in enumerate(target_codes):
        baseline_f1 = baseline_results['per_class_f1'][i]
        phase4_f1 = final_metrics['per_class_f1'][i]
        delta = phase4_f1 - baseline_f1
        print(f"  {code:<10} {baseline_f1:>10.4f} {phase4_f1:>10.4f} {delta:>+10.4f}")

    print(f"\n📊 Other Metrics:")
    print(f"  {'Metric':<20} {'Baseline':>10} {'Phase 4':>10}")
    print(f"  {'-'*20} {'-'*10} {'-'*10}")
    print(f"  {'Micro F1':<20} {baseline_results['micro_f1']:>10.4f} {final_metrics.get('micro_f1', 0):>10.4f}")
    print(f"  {'Precision':<20} {baseline_results['precision']:>10.4f} {final_metrics.get('precision', 0):>10.4f}")
    print(f"  {'Recall':<20} {baseline_results['recall']:>10.4f} {final_metrics.get('recall', 0):>10.4f}")

    print(f"\n📊 Concept Selection (Phase 4 only):")
    print(f"  Avg concepts activated: {final_metrics['avg_concepts']:.1f}")
    print(f"  Concept precision:      {final_metrics['concept_precision']:.1%}")

    print(f"\n⏱️  Training Time:")
    print(f"  Baseline:  {baseline_results['train_time']:.1f}s ({baseline_results['train_time']/60:.1f} min)")
    print(f"  Phase 4:   {total_time:.1f}s ({total_time/60:.1f} min)")
    print(f"  Ratio:     {total_time/baseline_results['train_time']:.1f}x")
