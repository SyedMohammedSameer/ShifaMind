#!/usr/bin/env python3
"""
ShifaMind Option A: High-Quality Concept Filtering
Building on 016.py (F1: 0.7759 → Target: 0.80+)

PROBLEM IDENTIFIED:
❌ Phase 1 (021.py): 300 concepts → only 61 with co-occurrence (20%) → F1 dropped to 0.7475
❌ Phase 2 (022.py): 500 concepts → only 85 with co-occurrence (17%) → F1 dropped to 0.6587
✅ 016.py Baseline: 150 concepts → likely ~80 with co-occurrence (53%) → F1 = 0.7759

ROOT CAUSE:
Adding concepts without co-occurrence = noise that hurts performance

SOLUTION (Option A):
1. Start with 150 concepts (like 016.py)
2. Build co-occurrence statistics from training data
3. FILTER: Keep only concepts with PMI > threshold (likely 80-100 concepts)
4. Rebuild embeddings with filtered high-quality concepts only
5. Train on clean, data-driven concept set

Expected Results:
- F1: 0.78-0.82
- Concepts: 80-100 (all high-quality)
- Precision: 80-90% (improved from 72.9%)
- Labels per sample: 10-15 (down from 17.4, more focused)

This is a TARGETED fix - remove noise, keep signal.
"""
...
class DiagnosisConditionalLabeler:
    ...
    def build_cooccurrence_statistics(self, df_train, target_codes):
        ...
        OPTION A: Returns filtered concepts with co-occurrence
        """
        ...
        print(f"  Unique concepts with co-occurrence: {len(self.concept_counts)}")
        print(f"  Total co-occurrences: {self.total_pairs}")

        # Compute PMI scores
        self._compute_pmi_scores()

        # OPTION A: Return filtered concepts
        filtered_concepts = set(self.concept_counts.keys())
        print(f"\n  🎯 OPTION A FILTER:")
        print(f"     Initial concepts: {len(self.concept_store.concepts)}")
        print(f"     Concepts with co-occurrence: {len(filtered_concepts)}")
        print(f"     Filtering ratio: {len(filtered_concepts)/len(self.concept_store.concepts)*100:.1f}%")
...
class Phase4ConceptStore:
    ...
    def filter_to_cooccurring_concepts(self, filtered_cuis: Set[str]):
        """
        OPTION A: Filter concept store to only keep concepts with co-occurrence

        Args:
            filtered_cuis: Set of CUIs that have co-occurrence in training data

        Returns:
            Number of concepts removed
        """
        print(f"\n🔍 OPTION A: Filtering concept store...")
        print(f"  Before: {len(self.concepts)} concepts")

        # Filter concepts
        concepts_to_remove = set(self.concepts.keys()) - filtered_cuis
        for cui in concepts_to_remove:
        ...
        print(f"  Removed: {len(concepts_to_remove)} noisy concepts")
        print(f"  ✅ Filtered to high-quality concepts only")
...
class Phase4RevisedTrainer:
    ...
    def train_stage1_diagnosis(self, epochs=3, lr=2e-5):
        ...
        for epoch in range(epochs):
            ...
            if val_metrics['macro_f1'] > best_f1:
                ...
                torch.save(self.model.state_dict(), 'stage1_diagnosis_optionA.pt')
    ...
    def generate_diagnosis_conditional_labels(self, df_train):
        ...
        labels = self.diagnosis_labeler.generate_dataset_labels(
            df_train,
            cache_file='diagnosis_conditional_labels_optionA.pkl'
        )
    ...
    def train_stage3_concepts(self, concept_labels, epochs=2, lr=2e-5):
        ...
        for epoch in range(epochs):
            ...
            torch.save(self.model.state_dict(), 'stage3_concepts_optionA.pt')
    ...
    def train_stage4_joint(self, concept_labels, epochs=3, lr=1.5e-5):
        ...
        for epoch in range(epochs):
            ...
            if val_metrics['macro_f1'] > best_f1:
                best_f1 = val_metrics['macro_f1']
                torch.save(self.model.state_dict(), 'stage4_joint_best_optionA.pt')
                print(f"  ✅ Best F1: {best_f1:.4f}")
...
"""
    ...
    plt.tight_layout()
    plt.savefig('optionA_results.png', dpi=300, bbox_inches='tight')
    print("\n✅ Saved: optionA_results.png")
    plt.show()
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("SHIFAMIND OPTION A: HIGH-QUALITY CONCEPT FILTERING")
    print("="*70)
    print("Building on 016.py - Removing noisy concepts that hurt performance")
    ...
    # Build co-occurrence statistics and get filtered concepts
    # OPTION A: This returns only concepts with actual co-occurrence
    filtered_concepts = diagnosis_labeler.build_cooccurrence_statistics(df_train, target_codes)

    # OPTION A: Filter concept store to high-quality concepts only
    concept_store.filter_to_cooccurring_concepts(filtered_concepts)
    ...
    # Create embeddings with FILTERED concepts
    print(f"\n🧬 Creating embeddings for {len(concept_store.concepts)} filtered concepts...")
    ...
    # Initialize model
    print("\n" + "="*70)
    print("INITIALIZING OPTION A MODEL (FILTERED CONCEPTS)")
    print("="*70)
    ...
    print(f"  Filtered concepts: {len(concept_store.concepts)} (high-quality only)")
    ...
    test_concept_labels = diagnosis_labeler.generate_dataset_labels(
        df_test_split,
        cache_file='diagnosis_conditional_labels_test_optionA.pkl'
    )
    ...
    model.load_state_dict(torch.load('stage4_joint_best_optionA.pt'))
    ...
    # Save artifacts
    print("\n💾 Saved artifacts:")
    print("  - stage1_diagnosis_optionA.pt")
    print("  - stage3_concepts_optionA.pt")
    print("  - stage4_joint_best_optionA.pt")
    print("  - diagnosis_conditional_labels_optionA.pkl")
    print("  - optionA_results.png")
    ...
    print("\n📋 Key Achievements:")
    print("  ✅ Filtered to high-quality concepts with co-occurrence")
    print("  ✅ Removed noisy concepts that hurt performance")
    print("  ✅ Diagnosis-conditional labeling with PMI")
    print("  ✅ Diagnosis-aware RAG filtering")
    print("  ✅ Staged training convergence")
    print(f"  ✅ Final concept count: {len(concept_store.concepts)}")
    ...
    if final_metrics['macro_f1'] > 0.7759:
        improvement = final_metrics['macro_f1'] - 0.7759
        print(f"\n🎯 SUCCESS! Improved F1 by {improvement:+.4f} from 016.py baseline")
    else:
        print(f"\n⚠️  Did not beat baseline 016.py (0.7759)")
