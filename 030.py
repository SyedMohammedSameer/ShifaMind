#!/usr/bin/env python3
"""
ShifaMind: Improved Forced Citation with Semantic Alignment
Standalone Colab Script - Complete Training + Evidence + Enhanced Reasoning Chains
...
# ============================================================================
# REASONING CHAIN GENERATOR - FIXED VERSION
# ============================================================================
# This cell replaces the original ReasoningChainGenerator class
# SAFE: Only affects demo visualization, not model performance

class ReasoningChainGenerator:
    """Generate reasoning chains (from 028.py) - FIXED CONCEPT SELECTION"""
    ...
    def generate_reasoning_chain(self, clinical_text, concept_embeddings):
        ...
        # ═══════════════════════════════════════════════════════════════════
        # FIX 1: EXPANDED DIAGNOSIS KEYWORDS (more comprehensive)
        # ═══════════════════════════════════════════════════════════════════
        diagnosis_keywords = {
            'J189': [
                'pneumonia', 'lung', 'respiratory', 'infection', 'infiltrate', 
                'bacterial', 'aspiration', 'pulmonary', 'chest', 'breath', 
                'cough', 'sputum', 'consolidation', 'alveolar', 'bronch',
                'pleural', 'thorax', 'ventilation', 'hypoxia'
            ],
            'I5023': [
                'heart', 'cardiac', 'failure', 'cardiomyopathy', 'edema', 
                'ventricular', 'atrial', 'cardio', 'myocardial', 'congestive',
                'hf', 'chf', 'ejection', 'dyspnea', 'orthopnea',
                'systolic', 'diastolic', 'chamber', 'output'
            ],
            'A419': [
                'sepsis', 'septicemia', 'bacteremia', 'infection', 'septic', 
                'shock', 'organ', 'systemic', 'inflammatory', 'sirs',
                'endotoxin', 'hypotension', 'fever', 'leukocytosis',
                'lactate', 'perfusion', 'dysfunction', 'failure'
            ],
            'K8000': [
                'cholecystitis', 'gallbladder', 'biliary', 'gallstone', 
                'cholelithiasis', 'bile', 'cholecyst', 'gall', 'hepatic',
                'choledoch', 'jaundice', 'bilirubin', 'duct', 'cystic'
            ]
        }

        # ═══════════════════════════════════════════════════════════════════
        # FIX 2: HYBRID CONCEPT SELECTION (guaranteed to show concepts)
        # ═══════════════════════════════════════════════════════════════════
        concept_list = list(self.concept_store.concepts.items())
        relevant_keywords = diagnosis_keywords.get(diagnosis_code, [])

        # Score ALL concepts with hybrid approach
        all_scored_concepts = []
        for idx, (cui, concept_data) in enumerate(concept_list):
            score = concept_probs[idx].item()
            concept_name_lower = concept_data['preferred_name'].lower()
            
            # Check diagnosis relevance
            is_relevant = any(kw in concept_name_lower for kw in relevant_keywords)
            
            # HYBRID: Include if relevant OR has decent score
            if is_relevant or score > 0.08:  # Relaxed threshold
                relevance_boost = 0.3 if is_relevant else 0.0
                all_scored_concepts.append({
                    'idx': idx,
                    'cui': cui,
        ...
        # Sort by final_score (model score + relevance boost)
        ...
        # ═══════════════════════════════════════════════════════════════════
        # FIX 3: GUARANTEED FALLBACK (never empty)
        # ═══════════════════════════════════════════════════════════════════
        if not scored_concepts:
            # Fallback: Take top 5 by raw model score alone
            print(f"⚠️ Using fallback: no relevant concepts for {diagnosis_code}")
            scored_concepts = sorted(
                [{'idx': idx, 'cui': cui, 'name': cd['preferred_name'], 
                  'score': concept_probs[idx].item(), 'is_relevant': False,
                  'final_score': concept_probs[idx].item()}
                 for idx, (cui, cd) in enumerate(concept_list)],
                key=lambda x: x['score'],
                reverse=True
            )[:5]

        # Build reasoning chain (unchanged logic)
        reasoning_chain = []
        for concept_info in scored_concepts:
            ...
            attn_weights = outputs['attention_weights'][-1]
            concept_attn = attn_weights[:, :, concept_idx]
            ...
            if evidence_spans:
                # Create claim with relevance indicator
                claim = self._create_claim(diagnosis_code, concept_data['preferred_name'])
                if concept_info.get('is_relevant', False):
                    claim = f"🎯 {claim}"  # Visual indicator for relevant concepts
                ...
                reasoning_chain.append({
                    ...
                    'concepts': [{
                        'cui': concept_cui,
                        'name': concept_data['preferred_name'],
                        'score': concept_score  # Use original score
                    }],
                    'evidence': evidence_spans,
                    'attention_scores': attention_scores
                })

        # RAG support (unchanged)
