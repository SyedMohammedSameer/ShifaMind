#!/usr/bin/env python3
"""
ShifaMind Option 2: Just More Epochs + Early Stopping
Building on 016.py (F1: 0.7759 → Last attempt: 0.78+)

LAST ATTEMPT - MINIMAL CHANGES:
✅ Same architecture (016.py works)
✅ Same concepts (150)
✅ Same everything else

ONLY CHANGES:
1. More epochs: 8 → 13 total
   - Stage 1: 3 → 5 epochs
   - Stage 3: 2 → 3 epochs
   - Stage 4: 3 → 5 epochs
2. Early stopping (patience=2)

Expected: F1 0.78-0.79 or we're done.
"""
...
class Phase4RevisedTrainer:
    ...
    def train_stage1_diagnosis(self, epochs=5, lr=2e-5):
        print("\n" + "="*70)
        print("STAGE 1: DIAGNOSIS HEAD TRAINING (5 EPOCHS + EARLY STOPPING)")
        print("="*70)
        ...
        for epoch in range(epochs):
            ...
            if val_metrics['macro_f1'] > best_f1:
                ...
                patience_counter = 0
                torch.save(self.model.state_dict(), 'stage1_diagnosis_option2.pt')
    ...
    def generate_diagnosis_conditional_labels(self, df_train):
        ...
        labels = self.diagnosis_labeler.generate_dataset_labels(
            df_train,
            cache_file='diagnosis_conditional_labels_option2.pkl'
        )
    ...
    def train_stage3_concepts(self, concept_labels, epochs=3, lr=2e-5):
        ...
        for epoch in range(epochs):
            ...
            torch.save(self.model.state_dict(), 'stage3_concepts_option2.pt')
    ...
    def train_stage4_joint(self, concept_labels, epochs=5, lr=1.5e-5):
        print("\n" + "="*70)
        print("STAGE 4: JOINT FINE-TUNING (5 EPOCHS + EARLY STOPPING)")
        print("="*70)
        ...
        for epoch in range(epochs):
            ...
            if val_metrics['macro_f1'] > best_f1:
                best_f1 = val_metrics['macro_f1']
                patience_counter = 0
                torch.save(self.model.state_dict(), 'stage4_joint_best_option2.pt')
                print(f"  ✅ Best F1: {best_f1:.4f}")
            else:
                patience_counter += 1
                print(f"  ⏸️  No improvement ({patience_counter}/{patience})")
...
"""
    ...
    plt.tight_layout()
    plt.savefig('option2_results.png', dpi=300, bbox_inches='tight')
    print("\n✅ Saved: option2_results.png")
    plt.show()
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("SHIFAMIND OPTION 2: JUST MORE EPOCHS")
    print("="*70)
    ...
    test_concept_labels = diagnosis_labeler.generate_dataset_labels(
        df_test_split,
        cache_file='diagnosis_conditional_labels_test_option2.pkl'
    )
    ...
    model.load_state_dict(torch.load('stage4_joint_best_option2.pt'))
    ...
    # Save artifacts
    print("\n💾 Saved artifacts:")
    print("  - stage1_diagnosis_option2.pt")
    print("  - stage3_concepts_option2.pt")
    print("  - stage4_joint_best_option2.pt")
    print("  - diagnosis_conditional_labels_option2.pkl")
    print("  - option2_results.png")
