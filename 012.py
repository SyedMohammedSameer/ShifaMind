#!/usr/bin/env python3
"""
ShifaMind Phase 1 Optimization: Fast & Efficient Training
Performance Optimizations:
1. Reduced concept set (50 → 200 concepts)
2. RAG disabled during training (inference only)
3. Single fusion layer (Layer 11)
4. Smaller sequence length (256 tokens)
5. Efficient batch processing
6. Simplified loss function
7. Smart caching
"""
...
# ============================================================================
# PHASE 1: STREAMLINED UMLS LOADER
# ============================================================================
class FastUMLSLoader:
    ...
    def load_concepts(self, max_concepts: int = 20000):
...
# ============================================================================
# PHASE 1: STREAMLINED CONCEPT STORE
# ============================================================================
class StreamlinedConceptStore:
    """Optimized concept store - Phase 1"""
    ...
    def build_concept_set(self, target_icd_codes: List[str],
        ...
        """Build streamlined concept set - Phase 1: 50 concepts"""
        ...
        for icd in target_icd_codes:
            ...
            for variant in variants:
                if variant in self.icd_to_cui:
                    ...
                    relevant_cuis.update(cuis[:10])  # Top 10 per code

        print(f"  Direct mappings: {len(relevant_cuis)} concepts")

        # Strategy 2: Keyword-based expansion (if needed)
        if len(relevant_cuis) < target_concept_count:
            diagnosis_keywords = {
                'J189': ['pneumonia', 'lung infection', 'respiratory'],
                'I5023': ['heart failure', 'cardiac failure'],
                'A419': ['sepsis', 'septicemia', 'infection'],
                'K8000': ['cholecystitis', 'gallbladder']
            }
    ...
    def create_concept_embeddings(self, tokenizer, model, device):
        """Create concept embeddings - batched for speed"""
        print("\n🧬 Creating concept embeddings...")

        concept_texts = []
        for cui, info in self.concepts.items():
            # Shorter text for faster encoding
            text = f"{info['name']}."
            if info['definition']:
                text += f" {info['definition'][:100]}"
        ...
        with torch.no_grad():
            for i in tqdm(range(0, len(concept_texts), batch_size), desc="  Encoding"):
                ...
                encodings = tokenizer(
                    ...
                    padding=True,
                    truncation=True,
                    max_length=128,  # Shorter for concepts
                    return_tensors='pt'
                ).to(device)
...
# ============================================================================
# PHASE 1: LIGHTWEIGHT RAG (Inference Only)
# ============================================================================
class LightweightRAG:
    """Simplified RAG for inference only - Phase 1"""
    ...
    def retrieve(self, query_embeddings: np.ndarray, k: int = 3):
        """Simple retrieval"""
...
# ============================================================================
# PHASE 1: OPTIMIZED MODEL
# ============================================================================
class SimpleCrossAttention(nn.Module):
    """Simplified cross-attention - Phase 1"""
    ...
    def forward(self, hidden_states, concept_embeddings, attention_mask=None):
        ...
        # Residual connection
        output = self.layer_norm(hidden_states + context)

        return output, attn_weights.mean(dim=1)

class OptimizedShifaMind(nn.Module):
    """Phase 1: Optimized ShifaMind - Fast Training"""

    def __init__(self, base_model, concept_store, num_classes, fusion_layer=11):
        super().__init__()
        ...
        self.fusion = SimpleCrossAttention(self.hidden_size, num_heads=4)
    ...
    def forward(self, input_ids, attention_mask, concept_embeddings):
        ...
        # Get fusion layer hidden states
        hidden_states = outputs.hidden_states[self.fusion_layer]

        # Apply concept fusion
        fused_hidden, attn_weights = self.fusion(
            hidden_states, concept_embeddings, attention_mask
        )

        # Get CLS representation
        cls_hidden = fused_hidden[:, 0, :]
...
# ============================================================================
# PHASE 1: SIMPLIFIED LOSS
# ============================================================================
class StreamlinedLoss(nn.Module):
    """Simplified loss function - Phase 1"""

    def __init__(self, alpha=0.7, beta=0.3):
        super().__init__()
        self.alpha = alpha  # Diagnosis weight
        self.beta = beta    # Concept weight
    ...
    def forward(self, diagnosis_logits, labels, concept_scores):
        ...
        # Concept sparsity loss (encourage top-k selection)
        concept_probs = torch.sigmoid(concept_scores)
        loss_concept = torch.mean(concept_probs)  # L1 regularization

        # Combined loss
        total_loss = self.alpha * loss_dx + self.beta * loss_concept
...
# VISUALIZATION
# ============================================================================
def plot_results(baseline_metrics, enhanced_metrics, target_codes):
    """Plot comparison"""
    ...
    axes[0].bar(x + width/2, enhanced_vals, width, label='ShifaMind', alpha=0.8)
    ...
    axes[1].bar(x - width/2, baseline_per_class, width, label='Baseline', alpha=0.8)
    axes[1].bar(x + width/2, enhanced_per_class, width, label='ShifaMind', alpha=0.8)
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("SHIFAMIND PHASE 1: OPTIMIZED TRAINING")
    print("="*70)
    ...
    umls_concepts = umls_loader.load_concepts(max_concepts=20000)
    ...
    # ========================================================================
    print("\n" + "="*70)
    print("BUILDING CONCEPT STORE (PHASE 1: 50 CONCEPTS)")
    print("="*70)

    concept_store = StreamlinedConceptStore(
        umls_concepts,
    ...
    concept_set = concept_store.build_concept_set(
        target_codes,
        icd10_descriptions,
        target_concept_count=50  # Phase 1: Small set
    )
    ...
    # Create datasets (max_length=256 for Phase 1)
    train_dataset = ClinicalDataset(X_train, y_train, tokenizer, max_length=256)
    val_dataset = ClinicalDataset(X_val, y_val, tokenizer, max_length=256)
    test_dataset = ClinicalDataset(X_test, y_test, tokenizer, max_length=256)

    # Create dataloaders (batch_size=8 for Phase 1)
    ...
    for batch in tqdm(train_loader, desc="Training Baseline"):
        input_ids = batch['input_ids'].to(device)
        attention_mask = batch['attention_mask'].to(device)
        labels = batch['labels'].to(device)
    ...
    print(f"  ⏱️  Baseline training time: {baseline_time:.1f}s")
    ...
    # ========================================================================
    # TRAIN OPTIMIZED SHIFAMIND
    # ========================================================================
    print("\n" + "="*70)
    print("TRAINING OPTIMIZED SHIFAMIND (PHASE 1)")
    print("="*70)

    model = OptimizedShifaMind(
        base_model=base_model,
        concept_store=concept_store,
        num_classes=len(target_codes),
        fusion_layer=11  # Single layer
    ).to(device)
    ...
    criterion = StreamlinedLoss(alpha=0.7, beta=0.3)
    ...
    # Load best model
    model.load_state_dict(torch.load('best_phase1_model.pt'))
    ...
    pct = (improvement / baseline_metrics['macro_f1']) * 100 if baseline_metrics['macro_f1'] > 0 else 0
    ...
    if len(pneumonia_indices) > 0:
        ...
        cls_embedding = outputs['logits'].detach().cpu().numpy()

        # Use base model to get proper query embedding
        with torch.no_grad():
    ...
    # ========================================================================
    print("\n💾 Saved artifacts:")
    print("  - best_phase1_model.pt (model checkpoint)")
    print("  - phase1_results.png (performance visualization)")

    print("\n" + "="*70)
    print("✅ PHASE 1 COMPLETE!")
    print("="*70)
    print("\nNext steps:")
    print("  - Phase 2: Increase concepts to 100")
    print("  - Phase 3: Enable RAG during training")
    print("  - Phase 4: Add multi-layer fusion")
