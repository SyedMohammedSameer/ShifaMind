#!/usr/bin/env python3
"""
ShifaMind Phase 2: Expanded Dataset + More Concepts
Building on 016.py (F1: 0.7759 → Target: 0.84+)
...
✅ 8604 samples from 4 ICD-10 codes

PHASE 2 IMPROVEMENTS:
1. ✅ Expanded Dataset: 8.6K → 15K-20K samples
   - J189 → J180-J188 (all pneumonia types)
   - I5023 → I500-I5033 (all heart failure types)
   - A419 → A410-A419 (all sepsis types)
   - K8000 → K800-K8009 (all cholecystitis types)

2. ✅ More Concepts: 150 → 500
   - Better keyword matching
   - More coverage per diagnosis

3. ✅ Enhanced RAG: 88 → 500+ documents
   - Include all 500 concept definitions
   - Better retrieval quality

4. ✅ Gradient Accumulation: batch_size=8, accum_steps=4
...
# ============================================================================
# PHASE 2: EXPANDED ICD CODE RANGES
# ============================================================================
def expand_icd_codes(base_codes: List[str]) -> Dict[str, List[str]]:
    """
    Expand base ICD codes to include related codes

    Phase 2: Use code ranges to get more training data
    """
    expansions = {
        'J189': [f'J18{i}' for i in range(9)],  # J180-J188
        'I5023': [f'I50{i:02d}' for i in range(34)],  # I500-I5033
        'A419': [f'A41{i}' for i in range(10)],  # A410-A419
        'K8000': [f'K800{i}' for i in range(10)]  # K8000-K8009
    }

    expanded = {}
    for base_code in base_codes:
        if base_code in expansions:
            expanded[base_code] = expansions[base_code]
        else:
            expanded[base_code] = [base_code]

    return expanded

def prepare_dataset(df_diag, df_adm, df_notes, icd_descriptions,
                   target_codes, min_samples_per_code=100, use_expansion=True):
    print("\n🔧 Preparing dataset...")

    # PHASE 2: Expand ICD codes
    if use_expansion:
        code_expansions = expand_icd_codes(target_codes)
        all_related_codes = []
        for base_code, related in code_expansions.items():
            all_related_codes.extend(related)

        print(f"  📊 Phase 2: Expanding ICD codes")
        for base_code, related in code_expansions.items():
            print(f"     {base_code} → {len(related)} codes ({related[0]}...{related[-1]})")
    else:
        code_expansions = {code: [code] for code in target_codes}
        all_related_codes = target_codes
    ...
    # Match any related code
    df['has_target'] = df['icd_codes'].apply(
        lambda codes: any(code in all_related_codes for code in codes)
    )
    df_filtered = df[df['has_target']].copy()

    # Map to base codes for labels
    def map_to_base_labels(codes):
        labels = []
        for base_code in target_codes:
            related = code_expansions[base_code]
            has_code = any(c in related for c in codes)
            labels.append(1 if has_code else 0)
        return labels

    df_filtered['labels'] = df_filtered['icd_codes'].apply(map_to_base_labels)

    # Balance by base code groups
    max_per_code = 5000  # PHASE 2: Increased from 3000
    balanced_indices = set()

    for base_code in target_codes:
        related_codes = code_expansions[base_code]
        code_indices = df_filtered[
            df_filtered['icd_codes'].apply(
                lambda x: any(c in related_codes for c in x)
            )
        ].index.tolist()
        ...
        if n_samples > 0:
            ...
            print(f"     {base_code}: {len(code_indices)} available, selected {n_samples}")
    ...
    print(f"  ✅ Dataset: {len(df_final)} samples (expanded from ~8.6K)")
...
class Phase4ConceptStore:
    """Phase 2: Expanded concept store - 500 high-quality concepts"""
    ...
    def build_concept_set(self, target_icd_codes: List[str],
                         ...
                         target_concept_count: int = 500):
        print(f"\n🔬 Building Phase 2 concept set (target: {target_concept_count})...")

        relevant_cuis = set()

        # Strategy 1: Direct ICD mappings (PHASE 2: Increased limit)
        for icd in target_icd_codes:
            ...
            for variant in variants:
                if variant in self.icd_to_cui:
                    ...
                    relevant_cuis.update(validated[:80])  # PHASE 2: 30 → 80

        print(f"  Direct mappings: {len(relevant_cuis)} concepts")

        # Strategy 2: Enhanced keyword expansion (PHASE 2: More keywords)
        diagnosis_keywords = {
            'J189': ['pneumonia', 'lung infection', 'respiratory infection',
                     ...
                     'respiratory distress', 'hypoxia', 'dyspnea',
                     'cough', 'sputum', 'chest pain', 'fever'],
            'I5023': ['heart failure', 'cardiac failure', 'cardiomyopathy',
                      'pulmonary edema', 'ventricular dysfunction',
                      'congestive', 'ejection fraction', 'left ventricular',
                      'cardiac output', 'myocardial', 'arrhythmia',
                      'coronary', 'ischemia', 'valve', 'mitral'],
            'A419': ['sepsis', 'septicemia', 'bacteremia', 'infection',
                     'septic shock', 'organ dysfunction',
                     'systemic inflammatory', 'hypotension', 'tachycardia',
                     'fever', 'leukocytosis', 'lactate', 'organ failure',
                     'blood culture', 'antibiotic'],
            'K8000': ['cholecystitis', 'gallbladder', 'biliary disease',
                      'gallstone', 'cholelithiasis',
                      'biliary tract', 'bile duct', 'cholangitis',
                      'jaundice', 'abdominal pain', 'murphy sign']
        }
...
class Phase4RevisedTrainer:
    ...
    def train_stage1_diagnosis(self, epochs=3, lr=2e-5, accumulation_steps=4):
        print("\n" + "="*70)
        print("STAGE 1: DIAGNOSIS HEAD TRAINING (PHASE 2: GRADIENT ACCUMULATION)")
        print("="*70)
        print(f"  Batch size: 8, Accumulation: {accumulation_steps}, Effective: {8*accumulation_steps}")

        optimizer = torch.optim.AdamW(self.model.parameters(), lr=lr, weight_decay=0.01)
        criterion = Phase4Loss(stage='diagnosis')

        num_training_steps = epochs * len(self.train_loader) // accumulation_steps
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, batch in enumerate(tqdm(self.train_loader, desc="Training")):
                ...
                loss = loss_dict['total'] / accumulation_steps  # PHASE 2: Scale loss
                loss.backward()
            ...
            if val_metrics['macro_f1'] > best_f1:
                ...
                torch.save(self.model.state_dict(), 'stage1_diagnosis_phase2.pt')
    ...
    def generate_diagnosis_conditional_labels(self, df_train):
        ...
        labels = self.diagnosis_labeler.generate_dataset_labels(
            df_train,
            cache_file='diagnosis_conditional_labels_phase2.pkl'
        )

        return labels

    def train_stage3_concepts(self, concept_labels, epochs=2, lr=2e-5, accumulation_steps=4):
        print("\n" + "="*70)
        print("STAGE 3: CONCEPT HEAD TRAINING (PHASE 2: GRADIENT ACCUMULATION)")
        print("="*70)
        ...
        for epoch in range(epochs):
            ...
            torch.save(self.model.state_dict(), 'stage3_concepts_phase2.pt')
    ...
    def train_stage4_joint(self, concept_labels, epochs=3, lr=1.5e-5, accumulation_steps=4):
        print("\n" + "="*70)
        print("STAGE 4: JOINT FINE-TUNING (PHASE 2: GRADIENT ACCUMULATION)")
        print("="*70)
        ...
        for epoch in range(epochs):
            ...
            if val_metrics['macro_f1'] > best_f1:
                best_f1 = val_metrics['macro_f1']
                torch.save(self.model.state_dict(), 'stage4_joint_best_phase2.pt')
                print(f"  ✅ Best F1: {best_f1:.4f}")
...
if __name__ == "__main__":
    print("\n" + "="*70)
    print("SHIFAMIND PHASE 4 REVISED: DIAGNOSIS-CONDITIONAL LABELING")
    print("="*70)
    ...
    df_train, target_codes = prepare_dataset(
        ...
        use_expansion=True  # PHASE 2: Enable ICD code expansion
    )

    # Build concept store (PHASE 2: 500 concepts)
    print("\n" + "="*70)
    print("BUILDING CONCEPT STORE (500 CONCEPTS - PHASE 2)")
    print("="*70)
    ...
    concept_set = concept_store.build_concept_set(
        target_codes,
        icd10_descriptions,
        target_concept_count=500  # PHASE 2: Increased from 150
    )
    ...
    # Initialize model
    print("\n" + "="*70)
    print("INITIALIZING PHASE 4 REVISED MODEL")
    print("="*70)
    ...
    test_concept_labels = diagnosis_labeler.generate_dataset_labels(
        df_test_split,
        cache_file='diagnosis_conditional_labels_test_phase2.pkl'
    )
    ...
    model.load_state_dict(torch.load('stage4_joint_best_phase2.pt'))
    ...
    # RESULTS
    print("\n" + "="*70)
    print("FINAL RESULTS - PHASE 4 REVISED")
    print("="*70)
    ...
    print(f"  Phase 4 Revised: {final_metrics['avg_concepts']:.1f} avg (precision: {final_metrics['concept_precision']:.1%})")

    # Visualization
    print("\n📊 Creating visualizations...")
    plot_phase4_revised_results(phase3_results, final_metrics, target_codes)
    ...
    # Save artifacts
    print("\n💾 Saved artifacts:")
    print("  - stage1_diagnosis_phase2.pt")
    print("  - stage3_concepts_phase2.pt")
    print("  - stage4_joint_best_phase2.pt")
    ...
    print("\n📋 Key Achievements:")
    print("  ✅ Diagnosis-conditional labeling with PMI")
    print("  ✅ Reliable 10-15 labels per sample")
    print("  ✅ Diagnosis-aware RAG filtering")
    print("  ✅ Staged training convergence")
    ...
    print("\n💡 FUTURE TODO: Attention-based self-supervision")
    print("   Consider using attention weights as pseudo-labels for")
    print("   self-supervised concept selection in future phases.")
