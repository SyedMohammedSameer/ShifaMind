#!/usr/bin/env python3
"""
ShifaMind: Evidence Extraction via Cross-Attention Analysis
Standalone Colab Script with Full Training Pipeline + Evidence Span Extraction

SYSTEM OVERVIEW:
Medical diagnosis prediction using Bio_ClinicalBERT with concept-enhanced architecture
and interpretable evidence extraction from clinical text.
...
- Target Macro F1: 0.7730 with 70.5% concept precision

NEW: EVIDENCE SPAN EXTRACTION
- Post-processing inference step (NO retraining required)
- Extracts text spans from clinical notes that support activated concepts
- Uses existing cross-attention weights from EnhancedCrossAttention module
- Identifies high-attention tokens and merges into meaningful spans
- Provides interpretability: which text evidence supports each concept

EVIDENCE EXTRACTION METHOD:
1. Run inference to get diagnosis + concept predictions + attention weights
2. For each activated concept (score > 0.7):
   - Aggregate attention weights across fusion layers
   - Identify top 15% attention tokens for that concept
   - Merge consecutive high-attention tokens into spans
   - Filter spans: min 5 tokens, max 50 tokens, merge if within 3 tokens
   - Return top-5 evidence spans per concept with attention scores
...
2. Build concept store (150 concepts)
3. Train ShifaMind model (or load pretrained weights)
4. Run evidence extraction on 100 test samples
5. Save results to 'evidence_spans_evaluation.json'
6. Visualize 3 examples with highlighted evidence spans
...
OUTPUT FORMAT:
{
  ...
  'evidence_extractions': [
    ...
    ...  # top-5 activated concepts
  ]
}

This is a complete standalone script for training + evidence extraction.
"""
