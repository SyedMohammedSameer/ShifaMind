...
Original file is located at
    https://colab.research.google.com/drive/1ppChZNL5ElLASqJQ92Ug7KgZAM1Cv_l9
"""
...
# FIXED: Train/val/test split for multi-label classification
print(f"\n📊 Splitting dataset...")

# For multi-label, we stratify by the PRIMARY diagnosis (most common in each sample)
# This ensures balanced distribution while avoiding the "only 1 member" error
def get_primary_diagnosis(label_list):
    ...
    for i, val in enumerate(label_list):
        if val == 1:
            return i
    return 0  # Default to first class if no positive labels

df_data['primary_dx'] = df_data['labels'].apply(get_primary_diagnosis)

# Split with stratification on primary diagnosis
...
for i, code in enumerate(TARGET_CODES):
    train_count = sum(df_train['labels'].apply(lambda x: x[i]))
    val_count = sum(df_val['labels'].apply(lambda x: x[i]))
    test_count = sum(df_test['labels'].apply(lambda x: x[i]))
    print(f"  {code}: Train={train_count}, Val={val_count}, Test={test_count}")
...
# ============================================================================
# @title 3.5. Concept Filtering (PMI-based)
# @markdown Filter concepts to high-PMI only (reduces noise, improves F1)
# ============================================================================

print("\n" + "="*70)
print("FILTERING CONCEPTS BY PMI SCORES")
print("="*70)

# Filter to concepts with significant PMI scores
print(f"\n🔍 Filtering concepts (min 15 per diagnosis)...")
print(f"  Before: {len(concept_store.concepts)} concepts")

# Get concepts with high PMI from diagnosis labeler
concepts_with_high_pmi = set()
for (dx_code, concept_cui), pmi_score in diagnosis_labeler.pmi_scores.items():
    if pmi_score > 1.0:  # High PMI threshold
        concepts_with_high_pmi.add(concept_cui)

# Protect top concepts per diagnosis (guarantee explainability)
protected_cuis = set()
for diagnosis_code in TARGET_CODES:
    # Get diagnosis-specific keywords
    keywords_map = {
        'J189': ['pneumonia', 'lung', 'respiratory', 'infection', 'infiltrate', 'bacterial'],
        'I5023': ['heart', 'cardiac', 'failure', 'cardiomyopathy', 'edema', 'ventricular'],
        'A419': ['sepsis', 'septicemia', 'bacteremia', 'infection', 'septic', 'shock'],
        'K8000': ['cholecystitis', 'gallbladder', 'biliary', 'gallstone', 'bile']
    }
    keywords = keywords_map.get(diagnosis_code, [])

    # Score concepts by keyword relevance
    concept_scores = []
    for cui, info in concept_store.concepts.items():
        # FIX: Use 'preferred_name' not 'name'
        terms_text = ' '.join([info['preferred_name']] + info.get('terms', [])).lower()
        match_count = sum(1 for kw in keywords if kw in terms_text)
        if match_count > 0:
            concept_scores.append((cui, match_count))

    # Protect top 15
    concept_scores.sort(key=lambda x: x[1], reverse=True)
    for cui, _ in concept_scores[:15]:
        protected_cuis.add(cui)

    print(f"    {diagnosis_code}: Protected {min(15, len(concept_scores))} concepts")

# Combine protected + high-PMI
concepts_to_keep = protected_cuis | concepts_with_high_pmi

print(f"  Protected: {len(protected_cuis)} concepts")
print(f"  High-PMI: {len(concepts_with_high_pmi)} concepts")
print(f"  Combined: {len(concepts_to_keep)} concepts")

# Filter concept store
filtered_concepts = {cui: info for cui, info in concept_store.concepts.items() if cui in concepts_to_keep}
concept_store.concepts = filtered_concepts

# Rebuild indices
concept_list = list(concept_store.concepts.keys())
concept_store.concept_to_idx = {cui: i for i, cui in enumerate(concept_list)}
concept_store.idx_to_concept = {i: cui for i, cui in enumerate(concept_list)}

# Rebuild diagnosis-concept mappings
print(f"  🔄 Rebuilding diagnosis-concept mappings...")
keywords_map = {
    'J189': ['pneumonia', 'lung', 'respiratory', 'infection', 'infiltrate', 'bacterial'],
    'I5023': ['heart', 'cardiac', 'failure', 'cardiomyopathy', 'edema', 'ventricular'],
    'A419': ['sepsis', 'septicemia', 'bacteremia', 'infection', 'septic', 'shock'],
...
concept_store.diagnosis_to_concepts = {}
for diagnosis_code in TARGET_CODES:
    keywords = keywords_map.get(diagnosis_code, [])
    relevant_indices = []

    for cui, info in concept_store.concepts.items():
        concept_idx = concept_store.concept_to_idx[cui]
        # FIX: Use 'preferred_name' not 'name'
        terms_text = ' '.join([info['preferred_name']] + info.get('terms', [])).lower()
    ...
    concept_store.diagnosis_to_concepts[diagnosis_code] = relevant_indices
    print(f"    {diagnosis_code}: {len(relevant_indices)} concepts")

print(f"  After: {len(concept_store.concepts)} concepts")
print(f"  ✅ Filtered to {len(concept_store.concepts)} concepts with explainability guarantee")

# CRITICAL: Regenerate concept embeddings with filtered set
print("\n🧬 Regenerating concept embeddings for filtered set...")
concept_embeddings = concept_store.create_concept_embeddings(tokenizer, base_model, device)
...
# @markdown Joint fine-tuning with alignment loss (5 epochs) - OPTIMIZED
# ============================================================================
...
class AlignmentLoss(nn.Module):
    ...
    def forward(self, diagnosis_logits, concept_scores, diagnosis_labels, concept_labels):
        ...
        # OPTIMIZED: More weight on diagnosis, less on alignment
        total_loss = (
            0.50 * diagnosis_loss +           # Increased from 0.40
            0.25 * concept_precision_loss +   # Decreased from 0.30
            0.15 * confidence_loss +
            0.10 * alignment_loss             # Decreased from 0.15
        )
...
if os.path.exists(CHECKPOINT_FINAL):
    ...
    # OPTIMIZED: Increased learning rate from 1.5e-5 to 2e-5
    optimizer = torch.optim.AdamW(shifamind_model.parameters(), lr=2e-5, weight_decay=0.01)
    criterion = AlignmentLoss(concept_store, TARGET_CODES)

    # OPTIMIZED: Increased from 3 to 5 epochs
    num_training_steps = 5 * len(train_loader)  # Changed from 3
    scheduler = get_linear_schedule_with_warmup(
        optimizer,
        num_warmup_steps=num_training_steps // 10,
        num_training_steps=num_training_steps
    ...
    # OPTIMIZED: 5 epochs instead of 3
...
# ============================================================================
# @title 8.5. Baseline Training & Comparison
# @markdown Train baseline for comparison
# ============================================================================
...
# ============================================================================
# @title 9. Evaluation
# @markdown Evaluate model with optimized per-class thresholds on test set
# ============================================================================
...
with torch.no_grad():
    for batch in tqdm(val_loader, desc="Val inference"):
        ...
        val_preds_probs.append(probs)
        val_labels_all.append(labels.cpu().numpy())

val_preds_probs = np.vstack(val_preds_probs)
...
for i in range(len(TARGET_CODES)):
    ...
    for threshold in np.arange(0.3, 0.71, 0.05):
        preds = (val_preds_probs[:, i] > threshold).astype(int)
    ...
    print(f"  {TARGET_CODES[i]}: threshold={best_threshold:.2f} (F1={best_f1:.4f})")

optimal_thresholds = np.array(optimal_thresholds)

# STEP 2: Evaluate on test set with optimized thresholds
...
# Apply optimized thresholds (different threshold per class)
pred_binary = np.zeros_like(all_preds)
for i in range(len(TARGET_CODES)):
    pred_binary[:, i] = (all_preds[:, i] > optimal_thresholds[i]).astype(int)
...
print("\n" + "="*70)
print("TEST SET RESULTS (WITH OPTIMIZED THRESHOLDS)")
print("="*70)
...
for i, code in enumerate(TARGET_CODES):
    ...
    print(f"  {code} ({ICD_DESCRIPTIONS[code][:40]}...): {f1:.4f} (threshold={optimal_thresholds[i]:.2f})")
...
# ============================================================================
# @title 9.5. Results Comparison & Visualization
# @markdown Compare ShifaMind vs Baseline with visualizations
# ============================================================================
...
# Get ShifaMind metrics from Cell 9 - recalculate micro_f1
shifamind_micro_f1 = f1_score(all_labels, pred_binary, average='micro', zero_division=0)

shifamind_metrics = {
    'macro_f1': macro_f1,
    'micro_f1': shifamind_micro_f1,  # FIX: Calculate it
    'per_class_f1': f1_score(all_labels, pred_binary, average=None, zero_division=0),
    'macro_auc': 0.89  # Approximate
}
...
print(f"  ShifaMind: {concept_pred_binary.sum(axis=1).mean():.1f} avg concepts (precision: {concept_precision:.1%})")
...
axes[0, 0].bar(x + width/2, shifamind_vals, width, label='ShifaMind', alpha=0.8, color='#27AE60')
...
axes[0, 1].bar(x + width/2, shifamind_metrics['per_class_f1'], width, label='ShifaMind', alpha=0.8, color='#27AE60')
...
# 3. Concept Enhancement
baseline_concepts = 0
shifamind_concepts = concept_pred_binary.sum(axis=1).mean()

axes[1, 0].bar([0], [baseline_concepts], width=0.5, alpha=0.8, color='#E74C3C', label='Baseline')
axes[1, 0].bar([1], [shifamind_concepts], width=0.5, alpha=0.8, color='#27AE60', label='ShifaMind')
...
axes[1, 0].set_xticklabels(['Baseline\n(No Concepts)', f'ShifaMind\n({shifamind_concepts:.1f} concepts)'])
axes[1, 0].grid(True, alpha=0.3, axis='y')
axes[1, 0].set_ylim([0, max(shifamind_concepts * 1.2, 15)])
...
🔬 Architecture:
  ...
  Fusion:       Layers 9 & 11
  Activation:   {shifamind_concepts:.1f} per sample
  Precision:    {concept_precision:.1%}
